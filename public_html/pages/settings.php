...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $checkboxMap = [
        ...
        'clerk' => ['clerk_enabled','clerk_social_google','clerk_social_facebook','clerk_social_phone','clerk_keep_legacy_login','clerk_webhook_secret','clerk_after_sign_in','clerk_after_sign_up'],
    ];
    ...
    <div class="flex-1">
        <form method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="bg-white rounded-xl shadow-sm border p-5 space-y-4">
                ...
                <p class="text-xs text-gray-500">Configure the mobile bottom navigation bar buttons. Drag to reorder. Max 5 buttons recommended.</p>
            ...
            const DEFAULT_NAV = [
                ...
                { icon: 'fa-phone-alt', label: 'Call', url: 'tel:<?= e($s['site_phone'] ?? '') ?>', type: 'link', active_style: '' },
            ...
            function initMobileNav() {
                ...
                if (!Array.isArray(_navBtns) || _navBtns.length === 0) _navBtns = [...DEFAULT_NAV];
                renderNavList();
            }
            ...
            function addMobileNavBtn() {
                if (_navBtns.length >= 6) { alert('Maximum 6 buttons allowed'); return; }
                _navBtns.push({ icon: 'fa-link', label: 'New', url: '/', type: 'link', active_style: '' });
                renderNavList();
            }
            ...
            <div class="bg-white rounded-xl shadow-sm border p-5 space-y-5">
                ...
                <div id="clerk-fields" class="space-y-4 <?= ($s['clerk_enabled'] ?? '0') !== '1' ? 'hidden' : '' ?>">
                    ...
                    <div class="bg-amber-50 rounded-xl p-4 space-y-3">
                        <h5 class="text-sm font-semibold text-amber-800 mb-1"><i class="fas fa-link mr-1"></i>Webhook Setup</h5>
                        <p class="text-xs text-amber-700">Step 1: Add this URL in <a href="https://dashboard.clerk.com/last-active?path=webhooks" target="_blank" class="underline font-medium">Clerk Dashboard → Webhooks</a>. Subscribe to: <code>user.created</code>, <code>user.updated</code>, <code>user.deleted</code></p>
                        <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs text-amber-700 mb-1">Step 2: After creating the webhook, copy the <strong>Signing Secret</strong> (starts with <code>whsec_</code>) and paste it below.</p>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Webhook Signing Secret</label>
                            <input type="password" name="clerk_webhook_secret" value="<?= e($s['clerk_webhook_secret'] ?? '') ?>"
                                   class="w-full px-3 py-2.5 border rounded-lg text-sm font-mono bg-white" placeholder="whsec_...">
                            <p class="text-xs text-gray-400 mt-1">This verifies webhook payloads are genuinely from Clerk. Leave empty to skip verification (not recommended for production).</p>
                        </div>
                    </div>

                    <!-- Diagnostic Link -->
                    <div class="flex items-center justify-between pt-1">
                        <p class="text-xs text-gray-400">If login isn't working, use the diagnostic tool to check your setup.</p>
                        <a href="<?= SITE_URL ?>/api/clerk-diag.php" target="_blank"
                           class="inline-flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-medium border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50">
