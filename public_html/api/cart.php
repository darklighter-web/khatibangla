    ...
    if ($action === 'get_variants') {
        ...
        foreach ($variants as $v) {
            if (($v['option_type'] ?? '') === 'variation' && floatval($v['absolute_price'] ?? 0) > 0) {
                ...
                $vrp = floatval($v['var_regular_price'] ?? $v['absolute_price'] ?? 0);
                if ($vrp > 0 && ($varRegPriceMax === null || $vrp > $varRegPriceMax)) $varRegPriceMax = $vrp;
            }
        }
        ...
        echo json_encode([
            ...
            'product' => [
                ...
                'regular_price' => floatval($varRegPriceMax ?? $product['regular_price']),
                'image' => getProductImage($product),
                'var_price_min' => $varPriceMin,
                'var_price_max' => $varPriceMax,
            ],
