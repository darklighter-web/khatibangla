...
<tr data-order-id="<?= $order['id'] ?>">
    ...
    <td data-col="invoice" style="white-space:nowrap">
        ...
        <span style="font-size:8px;background:#f3e8ff;color:#7c3aed;padding:1px 5px;border-radius:3px;font-weight:600">⏰ PRE</span>
        <?php endif; ?>
    </td>
    ...
    <td data-col="note">
        <?php $hasShipNote=!empty($order['notes']);$hasOrderNote=!empty($order['order_note']);$hasPanelNote=!empty($order['panel_notes'])||!empty($order['admin_notes']);
        if($hasShipNote||$hasOrderNote||$hasPanelNote):?>
            <?php if($hasShipNote):?><div style="font-size:10px;color:#ea580c;line-height:1.3;margin-bottom:2px" title="<?=e($order['notes'])?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#f97316;margin-right:3px;vertical-align:middle"></span><?=e(mb_strimwidth($order['notes'],0,30,'...'))?></div><?php endif;?>
            <?php if($hasOrderNote):?><div style="font-size:10px;color:#2563eb;line-height:1.3;margin-bottom:2px" title="<?=e($order['order_note'])?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#3b82f6;margin-right:3px;vertical-align:middle"></span><?=e(mb_strimwidth($order['order_note'],0,30,'...'))?></div><?php endif;?>
            <?php if($hasPanelNote):?><div style="font-size:10px;color:#16a34a;line-height:1.3" title="<?=e($order['panel_notes']??$order['admin_notes']??'')?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#22c55e;margin-right:3px;vertical-align:middle"></span><?=e(mb_strimwidth($order['panel_notes']??$order['admin_notes']??'',0,30,'...'))?></div><?php endif;?>
    ...
    <td data-col="products" style="cursor:pointer" onclick='showProductPopup(<?=htmlspecialchars(json_encode(array_map(function($i){return["name"=>$i["product_name"],"variant"=>$i["variant_name"]??"","qty"=>intval($i["quantity"]),"price"=>floatval($i["price"]??0),"image"=>!empty($i["featured_image"])?imgSrc("products",$i["featured_image"]):"","sku"=>$i["sku"]??""];}, $oItems)),ENT_QUOTES)?>, "<?=e($order["order_number"])?>")'> 
        ...
        <div style="display:flex;align-items:center;gap:4px;margin-top:2px">
            ...
            <div style="overflow:hidden"><div style="font-size:11px;color:#334155;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px"><?=e($item['product_name'])?></div><div style="font-size:9px;color:#94a3b8;white-space:nowrap"><?php if(!empty($item['variant_name'])):?><?=e(mb_strimwidth($item['variant_name'],0,16,'..'))?> · <?php endif;?>Qty: <?=intval($item['quantity'])?></div></div>
        </div>
        <?php endforeach;if(count($oItems)>2):?><div style="font-size:9px;color:#94a3b8;margin-top:2px">+<?=count($oItems)-2?> more</div><?php endif;?>
    </td>

    <td data-col="tags">
        <?php $tagColors=['REPEAT'=>'bg-orange-100 text-orange-700','URGENT'=>'bg-red-100 text-red-700','VIP'=>'bg-purple-100 text-purple-700','GIFT'=>'bg-pink-100 text-pink-700','COD VERIFIED'=>'bg-green-100 text-green-700','ADVANCE PAID'=>'bg-emerald-100 text-emerald-700','FOLLOW UP'=>'bg-blue-100 text-blue-700'];
        if($sr['total']>1):?><span class="tag-badge" style="background:#fff7ed;color:#c2410c;border:1px solid #fed7aa;margin-bottom:2px">Repeat</span><br><?php endif;
    ...
    <td data-col="upload">
        <?php if($__link&&$__cid):?>
            <a href="<?=$__link?>" target="_blank" style="font-size:10px;color:#2563eb;text-decoration:none;font-family:monospace;font-weight:500;white-space:nowrap" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'"><?=e(mb_strimwidth($__tid,0,16,'..'))?></a>
            <?php if(!empty($order['courier_status'])):$__cs=$order['courier_status'];$__csc='#94a3b8';
                ...
                <div style="font-size:9px;color:<?=$__csc?>;margin-top:1px;white-space:nowrap"><?=e(mb_strimwidth($__cs,0,14,'..'))?></div>
            <?php endif;?>
        <?php elseif($__cid):?><span style="font-size:10px;color:#64748b;font-family:monospace"><?=e(mb_strimwidth($__cid,0,16,'..'))?></span>
        <?php else:?><span style="color:#d1d5db">—</span><?php endif;?>
    </td>

    <td data-col="print" style="text-align:center;vertical-align:middle">
        <?php $__printed=intval($order['print_count']??0);?>
        <?php if($__printed>0):?><span style="color:#22c55e;font-size:14px" title="Printed <?=$__printed?> time(s)">✓</span>
        <?php else:?><span style="color:#d1d5db;font-size:11px">—</span><?php endif;?>
    </td>

    <td data-col="user">
        <div style="font-size:10px;font-weight:500;color:#334155;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60px" title="<?=e($__userDisplay)?>"><?=e(mb_strimwidth($__userDisplay,0,9,'..'))?></div>
    </td>

    <td data-col="source" style="white-space:nowrap">
        <span style="font-size:9px;font-weight:600;color:#64748b"><?=$srcLabel?></span>
    </td>

    <td data-col="shipping">
        <?php if(!empty($order['courier_name'])):?><div style="font-size:10px;font-weight:600;color:#475569;white-space:nowrap"><?=e(mb_strimwidth($order['courier_name'],0,14,'..'))?></div><?php endif;?>
        <?php if(!empty($order['notes'])):?><div style="font-size:9px;color:#ea580c;line-height:1.3;margin-top:1px" title="<?=e($order['notes'])?>"><?=e(mb_strimwidth($order['notes'],0,26,'...'))?></div>
        <?php elseif(empty($order['courier_name'])):?><span style="color:#d1d5db">—</span><?php endif;?>
    </td>

    <td class="om-action-cell" style="vertical-align:middle">
        <?php $__isProc=in_array($order['order_status'],['processing','pending']);?>
        <a href="<?=adminUrl('pages/order-view.php?order='.urlencode($order['order_number']))?>" class="order-open-link om-btn om-btn-open" data-oid="<?=$order['id']?>" <?=!$__isProc?'onclick="return openOrderModal(this.href,event)"':''?>>Open</a>
