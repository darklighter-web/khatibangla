...
<tr data-order-id="<?= $order['id'] ?>">
    ...
    <td data-col="invoice" style="white-space:nowrap">
        <a href="<?= adminUrl('pages/order-view.php?order='.urlencode($order['order_number'])) ?>" style="font-weight:600;color:#111827;font-size:13px;text-decoration:none" onmouseover="this.style.color='#2563eb'" onmouseout="this.style.color='#111827'"><?= e($order['order_number']) ?></a>
        <span class="dot-menu" onclick="toggleRowMenu(this,<?= $order['id'] ?>,'<?= e($order['order_number']) ?>')" style="margin-left:2px">⋮</span>
    </td>

    <!-- Customer: phone/name/address with icons like reference -->
    <td data-col="customer">
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-bottom:2px">
            <span style="color:#9ca3af;font-size:12px">👤</span>
            <span class="cust-name"><?= e(mb_strimwidth($order['customer_name'],0,20,'...')) ?></span>
        </div>
        <div style="display:flex;align-items:center;gap:5px">
            <span style="color:#9ca3af;font-size:12px">📍</span>
            <span class="cust-addr"><?= e(mb_strimwidth($order['customer_address'],0,26,'...')) ?></span>
        </div>
    </td>
    ...
    <td data-col="note">
        <?php
        $__noteTime = '<div style="font-size:11px;color:#9ca3af;margin-bottom:2px">Updated ' . timeAgo($order['updated_at']?:$order['created_at']) . '</div>';
        ...
        if($hasShipNote||$hasOrderNote||$hasPanelNote):?>
            <?php if($hasShipNote):?><div style="font-size:12px;color:#ea580c;line-height:1.4;margin-bottom:2px"><?=e(mb_strimwidth($order['notes'],0,50,'...'))?></div><?php endif;?>
            <?php if($hasOrderNote):?><div style="font-size:12px;color:#2563eb;line-height:1.4;margin-bottom:2px"><?=e(mb_strimwidth($order['order_note'],0,50,'...'))?></div><?php endif;?>
            <?php if($hasPanelNote):?><div style="font-size:12px;color:#16a34a;line-height:1.4"><?=e(mb_strimwidth($order['panel_notes']??$order['admin_notes']??'',0,50,'...'))?></div><?php endif;?>
        <?php else:?><div style="color:#d1d5db">-</div><?php endif;?>
    </td>
    ...
    <td data-col="products" style="cursor:pointer" onclick='showProductPopup(<?=htmlspecialchars(json_encode(array_map(function($i){return["name"=>$i["product_name"],"variant"=>$i["variant_name"]??"","qty"=>intval($i["quantity"]),"price"=>floatval($i["price"]??0),"image"=>!empty($i["featured_image"])?imgSrc("products",$i["featured_image"]):"","sku"=>$i["sku"]??""];}, $oItems)),ENT_QUOTES)?>, "<?=e($order["order_number"])?>")'> 
        <div style="display:inline-flex;align-items:center;gap:3px;margin-bottom:4px">
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-top:3px">
            <img src="<?=!empty($item['featured_image'])?imgSrc('products',$item['featured_image']):'data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 40 40%22><rect fill=%22%23f1f5f9%22 width=%2240%22 height=%2240%22/><text y=%22.75em%22 x=%22.15em%22 font-size=%2224%22>📦</text></svg>'?>" class="prod-thumb" loading="lazy" style="width:32px;height:32px;border-radius:6px;object-fit:cover;border:1px solid #e5e7eb;flex-shrink:0">
            <div style="overflow:hidden"><div style="font-size:12px;color:#374151;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:130px"><?=e($item['product_name'])?></div><div style="font-size:10px;color:#9ca3af;white-space:nowrap"><?php if(!empty($item['variant_name'])):?><?=e(mb_strimwidth($item['variant_name'],0,18,'..'))?> · <?php endif;?>Qty: <?=intval($item['quantity'])?></div></div>
        </div>
        <?php endforeach;if(count($oItems)>2):?><div style="font-size:10px;color:#9ca3af;margin-top:2px">+<?=count($oItems)-2?> more</div><?php endif;?>
    </td>

    <!-- Tags -->
    <td data-col="tags">
        <?php if($sr['total']>1):?><span class="tag-badge" style="background:#fff7ed;color:#c2410c;border:1px solid #fed7aa;margin-bottom:3px">Repeat</span><br><?php endif;
        foreach(array_slice($tags,0,2) as $tag):
            $tc2=$tagColors[$tag['tag_name']]??'bg-gray-100 text-gray-600';
        ?><span class="tag-badge <?=$tc2?>" style="margin-bottom:3px"><?=e($tag['tag_name'])?></span><br><?php endforeach;
        if(count($tags)>2):?><span style="font-size:10px;color:#9ca3af">+<?=count($tags)-2?></span><?php endif;?>
    </td>

    <!-- Total -->
    <td data-col="total" style="text-align:right;white-space:nowrap">
        <div style="font-weight:700;color:#111827;font-size:13px"><?=number_format($order['total'],2)?></div>
        <?php if($creditUsed>0):?><div style="font-size:10px;color:#ca8a04">-৳<?=number_format($creditUsed)?></div><?php endif;?>
    </td>
        ...
        <?php else: ?>
            <div style="display:flex;align-items:center;gap:6px;cursor:pointer">
                ...
                <div style="line-height:1.3">
                    ...
                    <div style="font-size:10px;color:#6b7280">Rating: <?= is_numeric($sr['customer_rating']) ? $sr['customer_rating'] : intval($sr['rate']*0.74) ?></div>
                    <?php elseif($sr['total']>0):?>
                    <div style="font-size:10px;color:#6b7280">Rating: <?= max(0,intval($sr['rate']*0.74)) ?></div>
                    <?php endif;?>
                </div>
            </div>
        <?php endif; ?>
    ...
    <td data-col="upload">
        <?php if($__link&&$__cid):?>
            <a href="<?=$__link?>" target="_blank" style="font-size:11px;color:#2563eb;text-decoration:none;font-family:monospace;font-weight:500;white-space:nowrap" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'"><?=e(mb_strimwidth($__tid,0,16,'..'))?></a>
            <?php if(!empty($order['courier_status'])):$__cs=$order['courier_status'];$__csc='#9ca3af';
                ...
                <div style="font-size:10px;color:<?=$__csc?>;margin-top:1px"><?=e(mb_strimwidth($__cs,0,14,'..'))?></div>
            <?php endif;?>
        <?php elseif($__cid):?><span style="font-size:11px;color:#6b7280;font-family:monospace"><?=e(mb_strimwidth($__cid,0,16,'..'))?></span>
    ...
    <!-- User -->
    <td data-col="user"><div style="font-size:11px;color:#374151;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:65px" title="<?=e($__userDisplay)?>"><?=e(mb_strimwidth($__userDisplay,0,10,'..'))?></div></td>
    ...
    <td data-col="shipping">
        <?php if(!empty($order['courier_name'])):?><div style="font-size:11px;font-weight:500;color:#374151"><?=e($order['courier_name'])?></div><?php endif;?>
        <?php if(!empty($order['notes'])):?><div style="font-size:10px;color:#ea580c;line-height:1.3;margin-top:1px"><?=e(mb_strimwidth($order['notes'],0,28,'...'))?></div>
    ...
    <td style="text-align:center;vertical-align:middle">
        ...
        <a href="<?=adminUrl('pages/order-view.php?order='.urlencode($order['order_number']))?>" class="order-open-link" data-oid="<?=$order['id']?>" <?=!$__isProc?'onclick="return openOrderModal(this.href,event)"':''?> style="display:inline-flex;align-items:center;gap:3px;padding:6px 14px;border-radius:6px;font-size:12px;font-weight:500;color:#16a34a;text-decoration:none;transition:background .15s" onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background='transparent'">Open <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>
