...
<tr data-order-id="<?= $order['id'] ?>">
    <td style="width:28px;vertical-align:middle"><input type="checkbox" name="order_ids[]" value="<?= $order['id'] ?>" class="order-check" onchange="updateBulk()"></td>
    ...
    <td>
        <div style="display:flex;align-items:center;gap:4px">
            <span class="cust-name"><?= e(mb_strimwidth($order['customer_name'],0,22,'...')) ?></span>
        </div>
            ...
            <span class="rate-wrap">
                ...
                <div class="rate-popup" onclick="event.stopPropagation()">
                    ...
                    <div style="margin-bottom:10px">
                        ...
                        <div style="font-size:11px;color:#374151"><?php $__bp=[]; foreach($__cBreak as $cb){$__bp[]='<b>'.e($cb['name']).'</b>: '.$cb['delivered'].'/'.$cb['total'];} echo implode('&nbsp;&nbsp;&nbsp;',$__bp); ?></div>
                    </div>
                    <?php endif; ?>
    ...
    <td>
        ...
        if ($hasShipNote || $hasOrderNote || $hasPanelNote): ?>
            <?php if($hasShipNote):?><div style="font-size:10px;color:#ea580c;line-height:1.3;margin-bottom:2px" title="<?= e($order['notes']) ?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#f97316;margin-right:3px;vertical-align:middle"></span><?= e(mb_strimwidth($order['notes'],0,32,'...')) ?></div><?php endif;?>
            <?php if($hasOrderNote):?><div style="font-size:10px;color:#2563eb;line-height:1.3;margin-bottom:2px" title="<?= e($order['order_note']) ?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#3b82f6;margin-right:3px;vertical-align:middle"></span><?= e(mb_strimwidth($order['order_note'],0,32,'...')) ?></div><?php endif;?>
            <?php if($hasPanelNote):?><div style="font-size:10px;color:#16a34a;line-height:1.3" title="<?= e($order['panel_notes']??$order['admin_notes']??'') ?>"><span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#22c55e;margin-right:3px;vertical-align:middle"></span><?= e(mb_strimwidth($order['panel_notes']??$order['admin_notes']??'',0,32,'...')) ?></div><?php endif;?>
        <?php else: ?>
            <span style="color:#d1d5db">—</span>
        <?php endif; ?>
    </td>

    <td style="cursor:pointer" onclick='showProductPopup(<?= htmlspecialchars(json_encode(array_map(function($i){return["name"=>$i["product_name"],"variant"=>$i["variant_name"]??"","qty"=>intval($i["quantity"]),"price"=>floatval($i["price"]??0),"image"=>!empty($i["featured_image"])?imgSrc("products",$i["featured_image"]):"","sku"=>$i["sku"]??""];}, $oItems)),ENT_QUOTES) ?>, "<?= e($order["order_number"]) ?>")'> 
        ...
        <div style="display:flex;align-items:center;gap:4px;margin-top:2px">
            ...
            <div style="overflow:hidden">
                <div style="font-size:11px;color:#334155;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px"><?= e($item['product_name']) ?></div>
    ...
    <td>
        <?php if($__link && $__cid): ?>
            <a href="<?= $__link ?>" target="_blank" style="font-size:10px;color:#2563eb;text-decoration:none;font-family:monospace;font-weight:500" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'"><?= e(mb_strimwidth($__tid,0,18,'..')) ?></a>
            ...
            ?>
                <div style="font-size:9px;color:<?= $__csc ?>;margin-top:1px"><?= e(mb_strimwidth($__cs,0,16,'..')) ?></div>
            <?php endif; ?>
        <?php elseif($__cid): ?>
            <span style="font-size:10px;color:#64748b;font-family:monospace"><?= e(mb_strimwidth($__cid,0,18,'..')) ?></span>
        <?php else: ?>
            <span style="color:#d1d5db">—</span>
        <?php endif; ?>
    </td>
    ...
    <td>
        <div style="font-size:10px;font-weight:500;color:#334155;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:65px" title="<?= e($__userDisplay) ?>"><?= e(mb_strimwidth($__userDisplay,0,10,'..')) ?></div>
    </td>
    ...
    <td>
        <?php if (!empty($order['courier_name'])): ?>
            <div style="font-size:10px;font-weight:600;color:#475569;white-space:nowrap"><?= e(mb_strimwidth($order['courier_name'],0,14,'..')) ?></div>
        <?php endif; ?>
        <?php if (!empty($order['notes'])): ?>
            <div style="font-size:9px;color:#ea580c;line-height:1.3;margin-top:1px" title="<?= e($order['notes']) ?>"><?= e(mb_strimwidth($order['notes'],0,30,'...')) ?></div>
