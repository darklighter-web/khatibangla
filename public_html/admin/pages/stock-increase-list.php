<?php
/**
 * Increase Stock List - History of all stock increases
 */
require_once __DIR__ . '/../../includes/functions.php';
require_once __DIR__ . '/../includes/auth.php';
$pageTitle = 'Increase Stock List';
...
$where = "WHERE sm.movement_type='in' AND DATE(sm.created_at) BETWEEN ? AND ?";
...
if ($sourceFilter) { $where .= " AND sm.reference_type=?"; $params[] = $sourceFilter; }
...
$pagination = paginate($total, $page, ADMIN_ITEMS_PER_PAGE, adminUrl('pages/stock-increase-list.php?') . http_build_query(array_filter(['search'=>$search,'warehouse'=>$warehouseFilter,'source'=>$sourceFilter,'from'=>$dateFrom,'to'=>$dateTo])));

$sourceLabels = ['purchase'=>'New Purchase','return'=>'Customer Return','production'=>'Production','transfer_in'=>'Transfer In','audit_adjustment'=>'Audit Adjustment','other'=>'Other'];
    ...
    <div class="p-4 border-b">
        <form class="flex flex-wrap gap-3 items-center">
            ...
            <select name="source" class="border rounded-lg px-3 py-2 text-sm">
                ...
                <option value="<?= $k ?>" <?= $sourceFilter===$k?'selected':'' ?>><?= $v ?></option>
                <?php endforeach; ?>
            </select>
            ...
            <a href="<?= adminUrl('pages/stock-increase-new.php') ?>" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 ml-auto">+ New Increase</a>
        </form>
    </div>

    <div class="px-4 py-3 bg-green-50 border-b flex items-center gap-6 text-sm">
        <span class="text-gray-600">Total Records: <strong class="text-gray-800"><?= number_format($total) ?></strong></span>
        <span class="text-gray-600">Total Units Added: <strong class="text-green-600"><?= number_format($totalQty) ?></strong></span>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center"><span class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-semibold">+<?= $item['quantity'] ?></span></td>
                    <td class="px-4 py-3"><span class="px-2 py-1 bg-gray-100 rounded-full text-xs"><?= $sourceLabels[$item['reference_type'] ?? ''] ?? ucfirst($item['reference_type'] ?? 'N/A') ?></span></td>
                ...
                <?php endforeach; ?>
                <?php if (empty($items)): ?>
                <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">No increase records found</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
